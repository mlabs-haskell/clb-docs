...
lang: en-US
description: Describes different options for how CLB can be used.
---
...
Depending on your needs you can use low-level [CLB's bare API](/use/lib)
or use high-level API by pairing it with [Atlas capabilities](/use/atlas)
...
Neither is there a notion of time, nor blocks.
...
If it is valid then the state immediately gets cranked,
moving into the next imaginary slot,
and the transaction is discarded.
The only information that is preserved is transaction datums.
If the transaction happens to be invalid
a negative validation result with the reason is returned to the submitter.

## Node emulator mode

In the [node emulator mode](/use/node)
(also can be referred to as the socket mode or the node mode)
...
that mimics a full-fledged Cardano node behavior to some extent.
The process maintains an IPC socket the way the node does,
so one can use Ouroboros mini-protocols directly
or pull in Ogmios to get a more usable API.
